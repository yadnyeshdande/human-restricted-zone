...
# CPU Optimization Constants - Senior Engineer Improvements
TARGET_FPS = 20  # Reduced from 30 to 20 FPS for CPU efficiency
FRAME_SKIP = 5   # Process every 5th frame
DETECTION_BUFFER_SIZE = 40  # ~2 seconds at 20 FPS
MIN_CONSECUTIVE_DETECTIONS = 8  # Minimum consecutive detections before triggering (2-3 seconds)
DETECTION_COOLDOWN = 10.0  # Seconds to wait after triggering before allowing another trigger
INCREASED_CONFIDENCE = 0.55  # Increased from 0.45 for fewer false positives
...
class AppConfig:
    ...
    model: Dict[str, Any] = field(default_factory=lambda: {"path": None, "confidence": INCREASED_CONFIDENCE})
...
class DetectionBuffer:
    """Maintains a sliding window of recent detection states to reduce false positives"""
    def __init__(self, size: int = DETECTION_BUFFER_SIZE):
        self.buffer = deque(maxlen=size)
        self.last_trigger_time = 0
        self.cooldown_period = DETECTION_COOLDOWN
        
    def add_detection(self, detected: bool):
        """Add a detection result (True/False) to the buffer"""
        self.buffer.append(detected)
    
    def should_trigger(self) -> bool:
        """
        Returns True if we should trigger the relay based on:
        1. Enough consecutive recent detections
        ...
        # Need at least MIN_CONSECUTIVE_DETECTIONS in buffer
        if len(self.buffer) < MIN_CONSECUTIVE_DETECTIONS:
            return False
        
        # Check if last N detections are all True
        recent = list(self.buffer)[-MIN_CONSECUTIVE_DETECTIONS:]
        if all(recent):
    ...
    def get_detection_percentage(self) -> float:
        """Returns percentage of positive detections in buffer"""
        if not self.buffer:
            return 0.0
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                # Frame rate control for CPU efficiency
                try:
                    time.sleep(1.0 / TARGET_FPS)
                except Exception:
                    time.sleep(0.033)  # fallback ~30fps
            except Exception as e:
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
                ...
                try:
                    ...
                    # Frame skipping: only process every FRAME_SKIP frames for CPU efficiency
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Detection buffer for temporal smoothing and false positive reduction
        ...
        try:
            ...
            if self.mode == "detection" and getattr(self, "detection_running", False):
                try:
                    ...
                    # Add detection result to buffer for temporal smoothing
                    self.detection_buffer.add_detection(any_alert)
                    
                    # Check if we should trigger relay (requires sustained detection)
                    if self.detection_buffer.should_trigger():
                        logger.warning("ALERT: Sustained person detection in restricted area!")
                        ...
                        try:
                            ...
                            self.last_alert_label.setText(f"Last alert: {ts} (Sustained Detection)")
                    ...
                    # Update status with detection buffer info
                    detection_pct = self.detection_buffer.get_detection_percentage()
                    status_text = f"Detection: {detection_pct:.0f}% buffer | "
                    if any_alert:
                        status_text += "Person in restricted area!"
                    else:
                        status_text += "Clear"
                    try:
            ...
            # Always update teaching video preview with the latest capture frame
            # This fixes the frame freeze issue
            if self.mode == "teaching" and not self.freeze_teaching_frame:
                try:
                    latest = None
                    ...
                    logger.debug("Teaching frame update error: %s", e)
        except Exception:
